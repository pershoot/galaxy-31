...
#define p3_pnl_pwr_enb	TEGRA_GPIO_PC6
#define p3_bl_enb		TEGRA_GPIO_PD4
#define p3_lvds_shutdown	TEGRA_GPIO_PB2
#define p3_hdmi_hpd	TEGRA_GPIO_PN7
#define p3_hdmi_enb	TEGRA_GPIO_PV5

/*panel power on sequence timing*/
#define p3_pnl_to_lvds_ms	0
#define p3_lvds_to_bl_ms	200
...
static int p3_disp1_check_fb(struct device *dev, struct fb_info *info);

#ifdef CONFIG_TEGRA_DC
static int p3_panel_enable(void)
{
	...
	gpio_set_value(p3_pnl_pwr_enb, 1);
	mdelay(p3_pnl_to_lvds_ms);
	gpio_set_value(p3_lvds_shutdown, 1);
	mdelay(p3_lvds_to_bl_ms);
	return 0;
}

static int p3_panel_disable(void)
{
	gpio_set_value(p3_lvds_shutdown, 0);
	gpio_set_value(p3_pnl_pwr_enb, 0);
	return 0;
}
